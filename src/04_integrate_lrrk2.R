...
meta <- fread("/Volumes/Extreme SSD/work_bidmc/PPMI/meta_data/PPMI_Curated_Data_Cut_Public_20240129.tsv") %>%
  filter(EVENT_ID == "BL" & type_mutation == "LRRK2") %>%
...
comm_id <- intersect(meta$PATNO, rownames(proteomics_human)) #352
comm_id <- intersect(comm_id, rownames(rna)) #251
# comm_id <- intersect(comm_id, rownames(urine_qc)) #223
# comm_id <- intersect(comm_id, rownames(metabolics_qc)) #141
...
bayfit <- tune.iClusterBayes(cpus = 6, 
                             ...
                             prior.gamma = rep(0.1, 2),
                             sdev = 0.5, pp.cutoff = 0.5)
date()
save.image(file = "/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/LRRK2_2omics.RData")
# load("/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/LRRK2_3omics.RData")
...
tiff("/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/BIC_deviance_lrrk2_2omics.tiff")
...
tiff("/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/posterior_probability_2omics.tiff", res = 300, height = 2500, width = 1500)
par(mfrow = c(2, 1))
...
plot(bayfit$fit[[k]]$beta.pp[[2]], xlab = "Feature", ylab = "Posterior probability", main = "WB Transcriptomics")
dev.off()
...
trunc.csf_proteomics <- trunc_func(scale.csf_proteomics, 4)
trunc.urine_proteomics <- trunc_func(scale.urine_proteomics, 4)
...
df.pheno$PD <- ifelse(df.pheno$cohort == 1, "PD", "Prodromal")

col_markers <- list(cluster = c("1" = "#C7522B", "2" = "#089392"),
                    PD_status = c("PD" = "#AE123A", "Prodromal" = "#66ACCF"),
                    SEX = c("Female" = "#E49183", "Male" = "#66ACCF"),
                    duration = colorRamp2(range(df.pheno$duration_yrs, na.rm = T), colors = c("#F4D166", "#24693D")),
...
tiff("/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/heatmap_lrrk2_3omics.tiff", res = 300, height = 2500, width = 2500)
...
chisq.test(table(df.pheno$cluster, df.pheno$SEX)) #p-value = 3.68e-06
chisq.test(table(df.pheno$cluster, df.pheno$PD)) #p-value = 0.1876
chisq.test(table(df.pheno$cluster, df.pheno$CSFSAA_new)) #p-value = 0.1931
t.test(age_at_visit ~ cluster, data = df.pheno) #p-value = 0.3578
t.test(duration_yrs ~ cluster, data = df.pheno) #p-value = 7.315e-07
t.test(log2_ptau ~ cluster, data = df.pheno) #p-value = 0.3566
t.test(log2_ttau ~ cluster, data = df.pheno) #p-value = 0.2955
t.test(log2_asyn ~ cluster, data = df.pheno) #p-value = 0.2502
t.test(log2_abeta ~ cluster, data = df.pheno) #p-value = 0.4184
t.test(mean_caudate ~ cluster, data = df.pheno) #p-value = 0.02745
t.test(mean_putamen ~ cluster, data = df.pheno) #p-value = 0.02659
t.test(mean_striatum ~ cluster, data = df.pheno) #p-value = 0.01766
wilcox.test(scopa_gi ~ cluster, data = df.pheno) #p-value = 0.2115
wilcox.test(updrs1_score ~ cluster, data = df.pheno) #p-value = 0.1603
wilcox.test(updrs2_score ~ cluster, data = df.pheno) #p-value = 0.009422
wilcox.test(updrs3_score ~ cluster, data = df.pheno) #p-value = 0.04703
wilcox.test(moca ~ cluster, data = df.pheno) #p-value = 0.7995
...
setwd("/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2")
...
pathway_enrichr(list = urine_signatures,
                dbs = "KEGG_2021_Human",
                plot.out = "/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/pathways/kegg_urine.tiff",
                plot.title = "LRRK2: Urine signatures",
                table.out = "/Volumes/Extreme SSD/work_bidmc/PPMI/iCluster/lrrk2/pathways/kegg_urine.tsv")
